...
parsingErrorFiles =
  ...
  , ("Error in $: parsing Machine.Machine(Machine) failed, key \"name\" not found", jsonBasePath </> "parsing_error_missing_keys2.json")
  ...
  , ("Error in $: parsing Machine.Machine(Machine) failed, unknown fields: [\"blakn\"]", jsonBasePath </> "parsing_error_typo_key.json")
  , ("Error in $: parsing Machine.Machine(Machine) failed, unknown fields: [\"?\",\"hmmm\"]", jsonBasePath </> "parsing_error_unexpected_keys.json")
  , ("Error in $.transitions.scanright[0]: parsing Machine.Transition(Transition) failed, key \"to_state\" not found", jsonBasePath </> "parsing_error_transition_to_state_empty.json")
  ]

validationErrorFiles :: [(String, FilePath)]
validationErrorFiles =
  ...
  , ("All elements in the alphabet must be a single character", jsonBasePath </> "validation_error_alphabet_contains_non_1_char_string.json")
  ...
  , ("Blank should be in the alphabet", jsonBasePath </> "validation_error_blank_is_empty.json")
  ...
  , ("Invalid write: 42 should be in the alphabet.\n", jsonBasePath </> "validation_error_transition_write_not_from_alphabet.json")
  , ("Invalid read: 42 should be in the alphabet.\n", jsonBasePath </> "validation_error_transition_read_not_from_alphabet.json")
...
parseMachineShouldFailWhenJsonIsIncorrect :: (String, String) -> Test
parseMachineShouldFailWhenJsonIsIncorrect (expectedErrorMsg, jsonFilePath) = TestCase $ do
...
validateMachineShouldFailWhenJsonIsIncorrect :: (String, String) -> Test
validateMachineShouldFailWhenJsonIsIncorrect (expectedErrorMsg, jsonFilePath) = TestCase $ do
  jsonFileContents <- BL.readFile jsonFilePath
  case parseMachine jsonFileContents of
    Left actualErrorMsg -> assertFailure "Parsing error, but should succeed"
    Right machine -> case validateMachine machine of
      Left validationError -> assertEqual "Error message mismatch" expectedErrorMsg validationError
      Right _ -> assertFailure "Expected failure, but parsing succeeded"

tests :: Test
tests =
  TestList $
    map (TestLabel "parseMachineShouldFailWhenJsonIsIncorrect" . parseMachineShouldFailWhenJsonIsIncorrect) parsingErrorFiles
      ++ map (TestLabel "validateMachineShouldFailWhenJsonIsIncorrect" . validateMachineShouldFailWhenJsonIsIncorrect) validationErrorFiles
